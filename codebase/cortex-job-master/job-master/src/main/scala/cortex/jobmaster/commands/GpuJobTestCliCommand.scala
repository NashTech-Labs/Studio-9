...
import cortex.jobmaster.jobs.example.SmokeGpuJobRunner
import cortex.jobmaster.modules.SettingsModule
import cortex.rpc.S3TaskRPC

class GpuJobTestCliCommand(mesosMaster: String, settings: SettingsModule)(implicit loggerFactory: JMLoggerFactory) extends CliCommand {
  ...
  override def execute(): Unit = {
    new SmokeGpuJobRunner().run(mesosMaster, taskRPC, jmConf.tasksVersion, jmConf.tasksRegistry)
  }
}
