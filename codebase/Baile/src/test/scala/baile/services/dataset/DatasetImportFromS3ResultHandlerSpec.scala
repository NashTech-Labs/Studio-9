...
import cortex.api.job.dataset.{ S3DatasetImportResponse, UploadedDatasetFile }
...
class DatasetImportFromS3ResultHandlerSpec extends BaseSpec with BeforeAndAfterEach {
  ...
  private val handler: ActorRef = system.actorOf(Props(
    new DatasetImportFromS3ResultHandler(
      cortexJobService,
      jobMetaService,
      dao,
      logger
  ...
  when(jobMetaService.readRawMeta(eqTo(jobId), any[String])).thenReturn(future(S3DatasetImportResponse(
  ...
  "DatasetImportFromS3ResultHandler#handle" should {
    ...
    "handle HandleJobResult message" in {
      ...
      handler ! HandleJobResult(
        ...
        rawMeta = Json.toJsObject(DatasetImportFromS3ResultHandler.Meta(
          datasetId = datasetId
        ))
      )
      expectMsgType[Unit]
