...
then
    ...
    echo "*** ERROR ***"
    echo "Cannot associate ${TRAVIS_BRANCH} with any existing environment"
    abort
fi
...
function initializeParameters() {
    ...
    envsubst < $(dirname $0)/mongo_migration-marathon.json.template > $(dirname $0)/mongo_migration-marathon.json
...
if [[ "$ENVIRONMENT" == "release" ]]; then
  echo "The release environment is currently not set up."
  exit 0
fi
