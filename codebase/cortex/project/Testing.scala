...
object Testing {
  ...
  private lazy val e2eSettings =
    inConfig(EndToEndTest)(Defaults.testSettings ++ Seq(
        // TODO should be somehow found dynamically without being dependent on testkit project path
        javaOptions ++= Seq(s"-Dlogback.configurationFile=${baseDirectory.value}/../testkit/src/main/resources/logback-e2e.xml")
      )) ++ Seq(
