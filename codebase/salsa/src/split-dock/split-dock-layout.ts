/*-----------------------------------------------------------------------------
 ...
 |----------------------------------------------------------------------------*/
import { ArrayExt, ChainIterator, IIterator, chain, each, empty, map, once, reduce } from '@phosphor/algorithm/lib';
import { ElementExt } from '@phosphor/domutils/lib';
import { Message, MessageLoop } from '@phosphor/messaging/lib';
import { BoxEngine, BoxSizer, Layout, LayoutItem, Title, Widget } from '@phosphor/widgets/lib';
 ...
 * and tabs as needed.
 */
export class SplitDockLayout extends Layout {
  /**
   * The renderer used by the dock layout.
   */
  readonly renderer: SplitDockLayout.IRenderer;

  private _spacing = 4;
  private _dirty = false;
  private _root: Private.LayoutNode | null = null;
  private _expandedLayout: Private.TabLayoutNode | null;
   ...
   */
  constructor(options: SplitDockLayout.IOptions) {
    super();
  ...
  tabs(): IIterator<SplitDockTab> {
    return this._root ? this._root.iterTabs() : empty<SplitDockTab>();
  }
   ...
   */
  restoreLayout(config: SplitDockLayout.ILayoutConfig): void {
    ...
    let mainConfig: SplitDockLayout.AreaConfig | null;
    if (config.main) {
   ...
   */
  addWidget(widget: Widget, options: SplitDockLayout.IAddOptions = {}): void {
    // Parse the options.
    let ref = options.ref || null;
    let mode = options.mode || 'split-right';
   ...
   */
  hitTestTabAreas(clientX: number, clientY: number): SplitDockLayout.ITabAreaGeometry | null {
    ...
    let tabNode = (this._expandedLayout || this._root).hitTestTabNodes(x, y);
    ...
    let { tab, top, left, width, height } = tabNode;
    ...
    return { tab, x, y, top, left, right, bottom, width, height };
  }
  ...
  private _setExpandedLayout(expandedLayout: Private.TabLayoutNode | null) {
    this._expandedLayout = expandedLayout;
    if (this._expandedLayout) {
      const expandedTab = this._expandedLayout.tab;
      each(this.tabs(), tab => {
        if (expandedTab === tab) {
          tab.show();
          tab.expanded = true;
  ...
  private _insertSplit(widget: Widget, ref: Widget | null, refNode: Private.TabLayoutNode | null, orientation: Private.Orientation, after: boolean): void {
    ...
    let tabNode = new Private.TabLayoutNode(this._createTab(widget.title));
  ...
  private _fit(): void {
    ...
    if (this._expandedLayout) {
      let limits = this._expandedLayout.fit(this._spacing, this._items);
  ...
  private _createTab(title: Title<Widget>): SplitDockTab {
    ...
    let tab = this.renderer.createTab(title);
...
/**
 * The namespace for the `SplitDockLayout` class statics.
 */
export namespace SplitDockLayout {
  /**
  ...
  interface IRenderer {
    /**
     * Create a new tab for use with a dock layout.
     *
     * @returns A new tab for a dock layout.
     */
...
namespace Private {
   ...
   */
  export function normalizeAreaConfig(config: SplitDockLayout.AreaConfig, widgetSet: Set<Widget>): SplitDockLayout.AreaConfig | null {
    let result: SplitDockLayout.AreaConfig | null;
    if (config.type === 'tab-area') {
      result = normalizeTabAreaConfig(config as SplitDockLayout.ITabAreaConfig, widgetSet);
    } else {
      result = normalizeSplitAreaConfig(config as SplitDockLayout.ISplitAreaConfig, widgetSet);
    }
    return result;
  }
   ...
   */
  export function realizeAreaConfig(config: SplitDockLayout.AreaConfig, renderer: SplitDockLayout.IRenderer): LayoutNode {
    let node: LayoutNode;
    if (config.type === 'tab-area') {
      node = realizeTabAreaConfig(config as SplitDockLayout.ITabAreaConfig, renderer);
    } else {
      node = realizeSplitAreaConfig(config as SplitDockLayout.ISplitAreaConfig, renderer);
    }
    return node;
  }
  ...
  export class TabLayoutNode {
    ...
    /**
     * The tab for the layout node.
     */
    readonly tab: SplitDockTab;
    ...
    iterAllWidgets(): IIterator<Widget> {
      return chain(once(this.tab), this.iterUserWidgets());
    }
     ...
     */
    iterTabs(): IIterator<SplitDockTab> {
      return once(this.tab);
    }
     ...
     */
    createConfig(): SplitDockLayout.ITabAreaConfig {
      let widget = this.tab.child;
      return { type: 'tab-area', widget };
    }
    ...
    fit(spacing: number, items: ItemMap): ElementExt.ISizeLimits {
      ...
      let tabItem = items.get(this.tab);

      // Lookup the widget layout item.
      let widgetItem = items.get(this.tab.child);

      // Lookup the tab and widget sizers.
      let [tabSizer, widgetSizer] = this.sizers;
    ...
    update(left: number, top: number, width: number, height: number, spacing: number, items: ItemMap): void {
      ...
      // Lookup the tab layout item.
      let tabItem: LayoutItem = items.get(this.tab);

      // Lookup the widget layout item.
      let widgetItem: LayoutItem = items.get(this.tab.child);
  ...
  export class SplitLayoutNode {
     ...
     */
    iterTabs(): IIterator<SplitDockTab> {
      let children = map(this.children, child => child.iterTabs());
      return new ChainIterator<SplitDockTab>(children);
    }
     ...
     * Create a configuration object for the layout tree.
     */
    createConfig(): SplitDockLayout.ISplitAreaConfig {
   ...
   */
  function normalizeTabAreaConfig(config: SplitDockLayout.ITabAreaConfig, widgetSet: Set<Widget>): SplitDockLayout.ITabAreaConfig | null {
   ...
   */
  function normalizeSplitAreaConfig(config: SplitDockLayout.ISplitAreaConfig, widgetSet: Set<Widget>): SplitDockLayout.AreaConfig | null {
    // Set up the result variables.
    let orientation = config.orientation;
    let children: SplitDockLayout.AreaConfig[] = [];
    let sizes: number[] = [];
   ...
   */
  function realizeTabAreaConfig(config: SplitDockLayout.ITabAreaConfig, renderer: SplitDockLayout.IRenderer): TabLayoutNode {
    // Create the tab for the layout node.
    let tab = renderer.createTab(config.widget.title);
   ...
   */
  function realizeSplitAreaConfig(config: SplitDockLayout.ISplitAreaConfig, renderer: SplitDockLayout.IRenderer): SplitLayoutNode {
