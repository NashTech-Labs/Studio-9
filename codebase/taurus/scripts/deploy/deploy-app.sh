...
function initializeParameters() {
    ...
    configuration_path=${configuration_base_path}/${ENVIRONMENT}/vars.env

    export $(aws s3 cp ${configuration_path} - | xargs) > /dev/null

    parameters=$(grep "=" $(dirname $0)/vars.env | grep -o "^[^=]*")
    ...
    sed -e "s/REPLACE_WITH_ENV_VARS/${env_vars_final}/" $(dirname $0)/marathon.json.template > $(dirname $0)/tmp_marathon.json
