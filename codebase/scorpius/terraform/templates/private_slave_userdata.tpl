...
runcmd:
  ...
  - echo "`date '+%b %d  %H:%M:%S'` [private_slave_userdata] Existing hostname is $hostn"
  ...
  - echo "`date '+%b %d  %H:%M:%S'` [private_slave_userdata] New hostname is $newhostn"
  - service rsyslog restart
  - echo "`date '+%b %d  %H:%M:%S'` $newhostn [private_slave_userdata] rsyslog service restarted.."
  - service ntpd restart
  - echo "`date '+%b %d  %H:%M:%S'` $newhostn [private_slave_userdata] ntpd service restarted.."
  ...
  - service ntpd restart
  - echo "`date '+%b %d  %H:%M:%S'` $newhostn [private_slave_userdata] ntpd service restarted.."
  - if [[ -f /tmp/registry.tar ]];then docker load -i /tmp/registry.tar; fi
  - DATE=`date +%Y%m%d%H%M%S`
  - set -o noglob
  - source /opt/cronjob_helper.sh
  - log_collector="* * * * * cat ${terraform_log_file} | grep -e ansible -e PRIVATE -e private_slave_userdata > /tmp/scorpius.log"
