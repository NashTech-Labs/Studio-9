...
runcmd:
  ...
  - echo "`date '+%b %d  %H:%M:%S'` [master_userdata] Existing hostname is $hostn"
  ...
  - echo "`date '+%b %d  %H:%M:%S'` [master_userdata] New hostname is $newhostn"
  - service rsyslog restart
  - echo "`date '+%b %d  %H:%M:%S'` $newhostn [master_userdata] rsyslog service restarted.."
  - service ntpd restart
  - echo "`date '+%b %d  %H:%M:%S'` $newhostn [master_userdata] ntpd service restarted.."
  ...
  - log_collector="* * * * * cat ${terraform_log_file} | grep -e ansible -e MASTER -e master_userdata > /tmp/scorpius.log"
