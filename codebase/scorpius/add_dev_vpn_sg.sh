...
if [[ $INCLUDE_REDSHIFT = "true" ]];then
    ...
    aws ec2 authorize-security-group-ingress --group-id ${REDSHIFT_SG} --protocol tcp --port 5439 --source-group ${DC_VPN_SG}
    echo "Access to redshift from Dev VPN has been granted."
fi
...
aws ec2 authorize-security-group-ingress --group-id ${UM_LB_SG} --protocol all --port all --source-group ${DC_VPN_SG}
echo "Access to the DeepCortex UI from Dev VPN has been granted."
